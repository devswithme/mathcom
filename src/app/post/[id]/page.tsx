...
const Page = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        (async () => {
          ...
          setLocalVotes({
            ...(userData.upvotedPosts || {}),
            ...(userData.upvotedComments || {}),
          });
        })();
      } else {
        setCurrentUser(null);
    ...
    const fetchPostAndComments = async () => {
      try {
        ...
        if (postSnap.exists()) {
          ...
          if (currentUser) {
            const isVoted = localVotes[postId] || false;
            setVoted(isVoted);
          }
          ...
          const commentVotesMap: Record<string, boolean> = {};

          commentsData.forEach((comment) => {
            if (localVotes[comment.id]) {
              commentVotesMap[comment.id] = true;
            }
          });

          setCommentVotes(commentVotesMap);
  ...
  const handleVote = async () => {
    ...
    try {
      await toggleVote({
        ...
        isVoted: voted, // this is the current state, so toggle accordingly
      });

      const newVoteState = !voted;
      const voteDelta = newVoteState ? 1 : -1;

      setVoted(newVoteState);
      setPost((prev) =>
        prev ? { ...prev, upvotes: prev.upvotes + voteDelta } : prev
      );
  ...
  const handleCommentVote = async (commentId: string) => {
    if (!currentUser || votingInProgress.current) return;

    votingInProgress.current = true;
    setLoadingId(commentId);

    const currentVoted = localVotes[commentId] || false;
    ...
    try {
      ...
      setCommentVotes((prev) => ({ ...prev, [commentId]: newVoteState }));
      setComments((prev) =>
        prev.map((c) =>
          c.id === commentId ? { ...c, upvotes: c.upvotes + voteDelta } : c
        )
      );
      setLocalVotes((prev) => ({ ...prev, [commentId]: newVoteState }));
    } catch (error) {
      console.error("Vote failed:", error);
      toast("Failed to update vote", {
        description:
