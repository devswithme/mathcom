...
export default function Home() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        const votes = userData.upvotedPosts || {};
        setVotedPosts(votes);
      } else {
        setCurrentUser(null);
        setVotedPosts({});
    ...
    const fetchPosts = async () => {
      ...
      const fetched = await Promise.all(
        snapshot.docs.map(async (docSnap) => {
          ...
          const userSnap = await getDoc(userRef);
          const userData = userSnap.exists() ? userSnap.data() : {};
          const userPhotoURL = userData.avatarUrl || "";
  ...
  const handleVote = async (e: React.MouseEvent, postId: string) => {
    ...
    if (!canVote || loadingVote === postId) return;

    setLoadingVote(postId);
    setCanVote(false);

    // Optimistic update values
    const previouslyVoted = votedPosts[postId] || false;
    const newVoteState = !previouslyVoted;
    ...
    setPosts((prev) =>
      prev.map((post) =>
        post.id === postId
          ? { ...post, upvotes: (post.upvotes || 0) + voteDelta }
          : post
      )
    );
    ...
    try {
      ...
      // Revert if the backend call failed
      ...
      setVotedPosts((prev) => ({ ...prev, [postId]: previouslyVoted }));
      setPosts((prev) =>
        prev.map((post) =>
          post.id === postId
            ? { ...post, upvotes: (post.upvotes || 0) - voteDelta }
    ...
    setLoadingVote(null);
    setTimeout(() => setCanVote(true), 1000); // shorter cooldown
  };
  ...
  return (
    <>
      <div className="flex w-full px-8 pt-[56px]">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-center gap-x-3">
                        ...
                        <div className="flex-1">
                          <h1 className="font-semibold text-sm">
                            m/
                            {post.community?.toLowerCase().replace(/\s/g, "_")}
                          </h1>
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: getPreviewHtml(post.description),
                        }}
                      />
